...
"""
Caudate-only ADHD classification.
Features: L_Caud_m* and R_Caud_m* shape coefficients (672 total).
...
feat_cols = [c for c in df.columns
             if c.startswith("L_Caud") or c.startswith("R_Caud")]
print(f"  Caudate features: {len(feat_cols)}  "
      f"(L_Caud: {sum(c.startswith('L_Caud') for c in feat_cols)}, "
      f"R_Caud: {sum(c.startswith('R_Caud') for c in feat_cols)})")
...
result.to_csv("/Users/devansh/adhd200_project/caudate_results.csv", index=False)
print("\nSaved: caudate_results.csv")
